...
LOCAL_PATH := device/3rd/sockit/bsp
...
BOARD_KERNEL_CMDLINE := dwc_otg.lpm_enable=0 console=ttyS0,115200 console=tty1 elevator=deadline rootwait androidboot.hardware=sockit androidboot.selinux=permissive
...
PRODUCT_COPY_FILES += \
  $(LOCAL_PATH)/init.sockit.rc:root/init.sockit.rc \
  $(LOCAL_PATH)/initnetwork.sh:system/bin/initnetwork.sh \
  system/core/rootdir/init.usb.rc:root/init.usb.rc \
  system/core/rootdir/ueventd.rc:root/ueventd.rc \
  $(LOCAL_PATH)/ueventd.sockit.rc:root/ueventd.sockit.rc \

BOARD_SEPOLICY_DIRS += \
	$(LOCAL_PATH)/sepolicy \

TARGET_KERNEL_SRC := hardware/bsp/kernel/gx/terasic_sockit_mtl_313
TARGET_KERNEL_DEFCONFIG := socfpga_defconfig
TARGET_KERNEL_CONFIGS := $(realpath $(LOCAL_PATH)/soc.kconf)
TARGET_KERNEL_DTB := socfpga_cyclone5_socdk.dtb
